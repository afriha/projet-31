...
router.post("/register", (req, res) => {
  ...
  span.setTag('Register', req.body.name);
  span.setTag(Tags.HTTP_URL, req.baseUrl);
  span.setTag(Tags.HTTP_METHOD, 'Post');
  span.setTag(Tags.SPAN_KIND_MESSAGING_PRODUCER, 'Register-Service');
    // Form validation
...
router.post("/login", (req, res) => {
  ...
  // Find user by email
    User.findOne({ email }).then(user => {
      ...
      if (!user.isVerified) {
        span.setTag(Tags.ERROR, true)
        span.setTag(Tags.HTTP_STATUS_CODE, 401);
        span.finish();
...
router.post('/confirmation', (req, res) => {
  const span = tracer.startSpan("Confirmation");
...
router.post('/resend', (req, res) => {
  const span = tracer.startSpan("Resend");
